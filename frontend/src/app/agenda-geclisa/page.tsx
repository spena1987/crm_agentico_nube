...
export default function AgendaGeclisaPage() {
  ...
  const [selectedPreId, setSelectedPreId] = useState<number>(969) // Default: Asesoramiento Quirúrgico
  const [prestadorInfo, setPrestadorInfo] = useState<{ nombre: string; matricula: string } | null>(null)
  ...
  // 1. Cargar lista de prestadores
  const cargarPrestadores = async () => {
    try {
      setCargandoPrestadores(true)
      const res = await fetch(`${BACKEND_URL}/api/geclisa/prestadores`)
      if (res.ok) {
        const data = await res.json()
        setPrestadores(data.prestadores || [])
      }
    } catch (e) {
      console.error('Error cargando catálogo de prestadores:', e)
    } finally {
      setCargandoPrestadores(false)
    }
  }

  // 2. Establecer prestador por defecto del usuario logueado
  useEffect(() => {
    cargarPrestadores()
    if (profile?.geclisa_pre_id) {
      setSelectedPreId(Number(profile.geclisa_pre_id))
    }
  }, [profile])

  // 3. Cargar turnos de la agenda
  const cargarAgenda = async (preIdToUse?: number, fechaToUse?: string) => {
    const pId = preIdToUse || selectedPreId
  ...
  const turnosFiltrados = useMemo(() => {
    return turnos.filter((t) => {
      ...
      if (search.trim()) {
        ...
        return matchNombre || matchFicha || matchDNI || matchOS
      }
      return true
    })
  }, [turnos, filtroEstado, search])

  // Estilos de badge de estado
  const getBadgeEstado = (estadoKey: string) => {
    switch (estadoKey) {
      case 'confirmado':
        return {
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-[var(--secondary)]">
                Consulta hospitalaria en tiempo real, gestión de estados clínicos y acceso a expedientes.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="flex flex-col">
            <label className="text-[10px] font-bold text-slate-400 uppercase mb-1 flex items-center gap-1">
              <Stethoscope size={12} className="text-blue-600" /> Prestador / Especialista
            </label>
            <select
              value={selectedPreId}
              onChange={(e) => setSelectedPreId(Number(e.target.value))}
              disabled={cargandoPrestadores}
              className="px-3.5 py-2 text-xs font-bold border border-[var(--border)] rounded-xl bg-slate-50 dark:bg-slate-800 text-[var(--foreground)] focus:outline-none focus:ring-2 focus:ring-blue-500/20 shadow-sm min-w-[240px]"
            >
              {prestadores.map((p) => (
                <option key={p.pre_id} value={p.pre_id}>
                  {p.nombre} {p.matricula ? `(Mat: ${p.matricula})` : ''}
                </option>
              ))}
            </select>
          </div>
          ...
          <button
            ...
            disabled={loading}
            className="self-end p-2.5 border border-[var(--border)] rounded-xl text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all shadow-sm"
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[var(--card)] px-5 py-3.5 rounded-2xl border border-[var(--border)] shadow-sm">
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            className="px-3.5 py-1.5 text-xs font-bold border border-[var(--border)] rounded-xl bg-slate-50 dark:bg-slate-800 text-[var(--foreground)] focus:outline-none focus:ring-2 focus:ring-blue-500/20"
          />
        </div>

        <div className="text-right">
          <p className="text-xs font-bold text-blue-600 capitalize">{fechaLegible}</p>
      ...
      <div className="flex items-center gap-3 bg-[var(--card)] border border-[var(--border)] px-4 py-2.5 rounded-2xl shadow-sm">
        ...
        <input
          type="text"
          placeholder="Buscar turno por nombre del paciente, número de Ficha, DNI u Obra Social..."
          value={search}
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-[var(--card)] rounded-2xl border border-[var(--border)] text-slate-400 space-y-2">
          ...
          <p className="text-xs text-[var(--secondary)]">
            No se encontraron turnos para la fecha seleccionada ({fecha}) con los filtros actuales.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {turnosFiltrados.map((t) => {
            const badge = getBadgeEstado(t.estado_key)
            const isUpdating = updatingTurnoId === t.turno_id

            return (
              <div
                key={t.turno_id}
                className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-4 shadow-sm hover:shadow-md transition-all flex flex-col justify-between space-y-3 relative group"
              >
                {/* Header de Tarjeta: Hora & Estado */}
                <div className="flex items-start justify-between gap-2 border-b border-[var(--border)] pb-2.5">
                  <div className="flex items-center gap-2">
                    <div className="px-2.5 py-1 rounded-xl bg-blue-600 text-white font-mono font-bold text-xs flex items-center gap-1 shadow-sm">
                      <Clock size={13} />
                      <span>{t.hora}</span>
                    </div>
                    {t.es_sobreturno && (
                  ...
                  <div className="relative">
                    <select
                      ...
                      className={`text-[11px] font-bold px-2.5 py-1 rounded-lg border appearance-none pr-6 cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all ${badge.bg}`}
                    >
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="space-y-1 text-[11px] text-[var(--secondary)]">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="truncate">{t.consultorio} • {t.servicio}</span>
                    </div>

                    {t.observaciones && (
                      <p className="text-[10px] italic text-slate-500 bg-slate-50 dark:bg-slate-800/60 p-1.5 rounded-lg border border-slate-100 dark:border-slate-700/60 line-clamp-2">
                        {t.observaciones}
                      </p>
                    )}
                  </div>
                ...
                <div className="pt-2 border-t border-[var(--border)] flex items-center justify-between gap-2">
                  <button
                    ...
                    className="flex-1 py-1.5 px-2 bg-blue-50 dark:bg-blue-950/40 hover:bg-blue-100 dark:hover:bg-blue-900/50 text-blue-700 dark:text-blue-300 rounded-xl text-[11px] font-bold transition-all border border-blue-200 dark:border-blue-800 flex items-center justify-center gap-1.5 shadow-sm"
                  ...
                  <a
                    href={`/chat`}
                    className="p-1.5 bg-emerald-50 dark:bg-emerald-950/40 hover:bg-emerald-100 text-emerald-700 dark:text-emerald-300 rounded-xl border border-emerald-200 dark:border-emerald-800 transition-all flex items-center justify-center"
                  ...
                  <a
                    ...
                    className="p-1.5 bg-purple-50 dark:bg-purple-950/40 hover:bg-purple-100 text-purple-700 dark:text-purple-300 rounded-xl border border-purple-200 dark:border-purple-800 transition-all flex items-center justify-center"
